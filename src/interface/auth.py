...
from src.system.tracing import update_current_trace
...
async def get_current_user(
    ...
    try:
        ...
        update_current_trace(user_id=str(user_id_int))
...
async def get_current_session(
    ...
    try:
        ...
        # Bind request-scoped auth context for logs and tracing.
        bind_context(user_id=session.user_id, session_id=session.id)
        request.state.user_id = session.user_id
        request.state.session_id = session.id
        update_current_trace(user_id=str(session.user_id), session_id=session.id)

...
async def lifespan(app: FastAPI):
    ...
    if langfuse_enabled:
        ...
        try:
            auth_check = None if settings.ENVIRONMENT.value == "test" else check_langfuse_auth()
            logger.info(
                ...
                client_ready=langfuse_client is not None,
            )
        except Exception as exc:
            logger.warning("langfuse_auth_check_failed", error=str(exc), exc_info=True)
    elif settings.LANGFUSE_TRACING_ENABLED:
        logger.warning(
            "langfuse_tracing_requested_but_unavailable",

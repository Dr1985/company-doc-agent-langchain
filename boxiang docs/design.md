...
| **Phase 3.5: 前端 Chat UI** | 基于 Vue 3 + Vite 构建可直接使用的聊天界面：消息气泡、Markdown/代码块渲染、流式输出、引用来源卡片、会话列表与切换、登录态保持、空状态/错误态/重试、响应式布局，并对接文档列表、检索、聊天与历史接口 | 用户可在浏览器完成登录、提问、查看流式答案和引用来源，并浏览对话历史 | 📋 规划中 |

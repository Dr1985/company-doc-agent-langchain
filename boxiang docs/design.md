...
> **开发原则：前后端一体化**
>
> 1. **Phase 1 / Phase 2** 可以先用 Swagger UI、OpenAPI 或测试脚本验证接口，不强制先做前端页面。
> 2. **Phase 3 及之后** 的每个阶段，都要先产出对应的 **Vue 3 页面原型 / HTML 交互稿**，并在你确认后，再实现该阶段对应的后端 API 和业务逻辑。
> 3. 前端和后端视为同一阶段的整体交付，避免只做“没有后端的前端”，也避免先做出缺少前端验证的复杂后端。
> 4. 原独立的前端 Chat UI 需求已拆分并并入 **Phase 3 ~ Phase 6** 的具体交付中。

| 阶段 | 内容 | 关键交付 | 状态 |
|---|---|---|---|
| **Phase 1: 基础设施** | 搭建项目骨架、数据库、Langfuse、Prometheus | Docker Compose 一键启动所有服务；基础接口可通过 Swagger UI 验证 | ✅ 已完成 |
| **Phase 2: 文档入库** | PDF/Word/MD 解析、智能分块、元数据提取、表格提取、Embedding 入库、增量更新 | 文档上传 → 自动入库 → 可检索，支持增量重建索引；相关接口可通过 Swagger UI 验证 | ✅ 已完成 |
| **Phase 3: 基础问答** | RAG 流程（混合检索 + 父文档召回）、单轮问答、引用溯源、拒答机制；**先做 Vue 3 聊天页 / 问答页原型，确认后再做后端 API** | 用户提问 → 返回带来源的答案 | 🚧 进行中 |
| **Phase 4: 多模型路由** | 接入多模型、实现智能路由、fallback 策略、热点问答 Redis 缓存；**先做 Vue 3 路由配置页 / 缓存状态页原型，确认后再做后端 API** | 不同问题走不同模型，高频问题直接缓存命中 | 📋 规划中 |
| **Phase 5: 高级功能** | 多轮对话增强、RBAC 权限控制、OCR + 表格提取、Rerank 重排序、自动分类打标、文档摘要、相似文档推荐、敏感信息检测脱敏；**先做 Vue 3 管理功能页原型，确认后再做后端 API** | 完整的生产级功能 | 📋 规划中 |

...
DEFAULT_LLM_MODEL=                  # Leave blank to use provider defaults
DEFAULT_LLM_TEMPERATURE=0.2         # Controls randomness (0.0 = deterministic, 1.0 = creative)

# Optional: long-term memory embedding config
...
The application can now auto-select the chat provider during startup. If only `DEEPSEEK_API_KEY` is set, it will use DeepSeek. If only `OPENAI_API_KEY` is set, it will use OpenAI. If both are set, `LLM_PROVIDER` lets you force the provider explicitly.

One important caveat is long-term memory: the chat model can move to DeepSeek, but memory embeddings still need a compatible embedding provider. If you do not configure an embedder, long-term memory is disabled gracefully instead of crashing the app.

If you want to use Alibaba Cloud BaiLian's `text-embedding-v4`, keep the embedder provider as `openai` only if you are using Alibaba's OpenAI-compatible endpoint, then set:

```dotenv
LONG_TERM_MEMORY_EMBEDDER_BASE_URL=<Alibaba OpenAI-compatible endpoint>
LONG_TERM_MEMORY_EMBEDDER_MODEL=text-embedding-v4
LONG_TERM_MEMORY_EMBEDDER_DIMS=<the dimension you chose in BaiLian>
```

The embedding dimension must match the value stored in pgvector, otherwise the memory store can fail when it creates or queries the vector table.
